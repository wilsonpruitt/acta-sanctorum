...
async function main() {
  const outDir = join(ROOT, 'src/latin/apr');
  await mkdir(outDir, { recursive: true });

  // Scrape intros
  for (const vol of ['I', 'II', 'III']) {
    const url = `${BASE_URL}/Einleitung_April_${vol}.html`;
    console.log(`Fetching Apr Vol ${vol} Introduction...`);
    try {
      const html = await fetchPage(url);
      const text = extractText(html);
      await writeFile(join(outDir, `einleitung-apr-${vol.toLowerCase()}.txt`), text, 'utf-8');
  ...
  for (let day = 1; day <= 30; day++) {
    const url = `${BASE_URL}/${day}.April.html`;
    console.log(`Fetching Apr ${day}...`);
    try {
      const html = await fetchPage(url);
      const text = extractText(html);
      ...
      console.log(`  Apr ${day}: ${words} words`);
    } catch (e) { console.error(`  Error on Apr ${day}: ${e.message}`); }
    await sleep(DELAY_MS);
  }
