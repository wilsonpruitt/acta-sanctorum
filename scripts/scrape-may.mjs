...
async function main() {
  const outDir = join(ROOT, 'src/latin/may');
  await mkdir(outDir, { recursive: true });

  // Scrape intros (May has 7 volumes)
  for (const vol of ['I', 'II', 'III', 'IV', 'V', 'VI', 'VII']) {
    const url = `${BASE_URL}/Einleitung_Mai_${vol}.html`;
    console.log(`Fetching May Vol ${vol} Introduction...`);
    try {
      const html = await fetchPage(url);
      const text = extractText(html);
      await writeFile(join(outDir, `einleitung-may-${vol.toLowerCase()}.txt`), text, 'utf-8');
  ...
  for (let day = 1; day <= 31; day++) {
    const url = `${BASE_URL}/${day}.Mai.html`;
    console.log(`Fetching May ${day}...`);
    try {
      const html = await fetchPage(url);
      const text = extractText(html);
      ...
      console.log(`  May ${day}: ${words} words`);
    } catch (e) { console.error(`  Error on May ${day}: ${e.message}`); }
    await sleep(DELAY_MS);
  }
