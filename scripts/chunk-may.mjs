...
async function chunkDay(day) {
  ...
  const srcPath = join(ROOT, `src/latin/may/day-${dp}/day-${dp}-full.txt`);
  const chunkDir = join(ROOT, `src/latin/may/day-${dp}/chunks`);
  if (!existsSync(srcPath)) { console.log(`May ${day}: skipped`); return 0; }
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    await writeFile(join(chunkDir, `${num}-may-day-${dp}.md`), chunks[i], 'utf-8');
  }
  console.log(`May ${dp}: ${chunks.length} chunks (${text.split(/\s+/).length} words)`);
